...
def main(nelx, nely, volfrac, penal, rmin):
    ...
    while change > 0.01 and loop < 2000:
        ...
        den_sens.vector.array = np.multiply(den_node.vector.array, sens_node)
        # HELMHOLTZ FILTERING ---------------------------------
        ...
        sens = np.divide(density_til.vector.array, np.maximum(1e-3, density.vector.array))
        ...
        while l2 - l1 > 1e-4:
            ...
            density_new = np.maximum(0.001, np.maximum(density.vector.array - move, np.minimum(1.0, np.minimum(density.vector.array + move, density.vector.array * np.sqrt(-sens/l_mid)))))
        ...
        print("it.: {0} , obj.: {1:.3f} Vol.: {2:.3f}, ch.: {3:.3f}, pros.:{4}\n".format(loop, sum(objective.vector.array), sum(density.vector.array)/num_cells, change, MPI.COMM_WORLD.rank))
