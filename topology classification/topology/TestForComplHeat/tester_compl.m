...
volfrac = 0.3;
x1 = xlsread("compl_test_1_1.xlsx");
[c0, c1] = topology(NODE, ELEM, volfrac, penal, x1);
disp(['  Start compliance.: ' sprintf('%g',c0) '  Finish compliance.: '  sprintf('%g',c1) '  ratio.: ' sprintf('%g',c1/c0)]);


%% Function
function [c0, c1] = topology(NODE, ELEM, volfrac, penal, x1)
    nele = length(ELEM);
    
    x0(1:nele) = volfrac;
    [U, KE] = FE_(NODE, ELEM, x0, penal);
    Ue = zeros(8, 1);
    c0 = 0;
    for i = 1:nele
        for j = 1:4
        ...
        c0 = c0 + (x0(i) ^ penal) * Ue' * KE(:, :, i) * Ue;
    end

    zero_components = find(x1 == 0);
    x1(zero_components) = 0.001;
    x1 = reshape(x1,[1,nele]);
    [U, KE] = FE_(NODE, ELEM, x1, penal);
    Ue = zeros(8, 1);
    c1 = 0;
    for i = 1:nele
        for j = 1:4
...
%     BC(x_upper, :) = 1;
    ...
    %% Force condition %%
%     BC_N(x_middle_up & y_middle_low, 1) = 1; % % % % % % % % % % % % % % % % % % % % % % % % % % %
    BC_N(x_middle_up & y_middle_low, 2) = -1;
