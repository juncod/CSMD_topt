...
% top3d_MMA3D(60,20,4,0.3,3,1.5,1)
function top3d_MMA3D(nelx,nely,nelz,volfrac,penal,rmin,qmax)
...
xPhys = x; baseplate='S';
[xPrint] = AM_filter3D(xPhys,baseplate);
loop = 0; 
change = 1;
...
% START ITERATION
Objj=zeros(maxloop,1);Voll=zeros(maxloop,1);Chh=zeros(maxloop,1);
while change > tolx && loop < maxloop
    ...
    dc(:) = H*(dc(:)./Hs); % ==> (22)
    dv(:) = H*(dv(:)./Hs); % ==> (19)
    [xPrint, dc,dv] = AM_filter3D(xPhys,baseplate,dc,dv);
    xval  = x(:);
    f0val = c;
    df0dx = dc(:);
    ...
    x = xnew;
    Objj(loop)=c;Voll(loop)=mean(xPrint(:));Chh(loop)=change;
    % PRINT RESULTS
    fprintf(' It.:%5i Obj.:%11.4f Vol.:%7.3f ch.:%7.3f\n',loop,c,mean(xPrint(:)),change);
    % PLOT DENSITIES
    if displayflag, clf; display_3D(xPrint); end %#ok<UNRCH>
end
clf; display_3D(xPrint);

% plot(1:loop,Objj(1:loop),'b-');

% plot(1:loop,Voll(1:loop),'r-');

% plot(1:loop,Chh(1:loop),'g-');
end
...
face = [1 2 3 4; 2 6 7 3; 4 3 7 8; 1 5 8 4; 1 2 6 5; 5 6 7 8]; % ==> 5-6-7-8
set(gcf,'Name','ISO display','NumberTitle','off');             %     | | | |
for k = 1:nelz                                                 %     1-2-3-4
    z = (k-1)*hz;
    for i = 1:nelx
        x = (i-1)*hx;
        for j = 1:nely
